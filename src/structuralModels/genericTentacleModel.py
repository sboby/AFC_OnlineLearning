...
# tentacle = ComplexTentacle( N, L, r, dt, duration)

 
# for i in range(1): 
#     output = tentacle.integrateTimeStep( dt )
#     # print("displacements:", output[0])
#     # print("velocity:", output[0])
#     tentacle.timeStep += 1 
#     tentacle.t += tentacle.dt
...
class PythonInterface ():
  ...
  def integrateTimeStep ( self, densityArray, velocityArray, dt  ):
    ...
    if self.timeIndex % timeStepSkipPlot == 0:
      ...
      # concentrationPlotAx.set_aspect ( particlePlotAx.get_aspect () )
      ...
      # self.fig.suptitle ( str(self.t)[:5] )

      buf = BytesIO()
      plt.savefig(buf, dpi = 200, format = None) # you may need "PNG" as type here
      buf.seek(0)
      im = Image.open(buf)
    ...
    #   np.savetxt('tentacleDataset_2.07_vel.txt', exportData, header = "v0, v1, v2")
  ...
  def finaliseTimeStep ( self, dt ):
    # return self.tentacle.finaliseTimeStep ( dt )
    # return self.beam.integrateTimeStep ( dt )
    pass